...
Once we declare a variable, whether we assign a value to it or not, like: `int num` or `int num = 5` — that memory space is no longer free. Other software can't use it; it's reserved for **us**. From that point on, we can use it however we like, as long as we don’t try to store a value that exceeds the amount of memory allocated for it.

#### `char` 
A char variable always occupies exactly 1 byte (8 bits) of memory, regardless of the specific value it holds. Whether it stores a small number like 5, a character like 'z' or none, the memory allocation remains the same.
